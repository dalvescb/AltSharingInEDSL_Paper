...
\mintinline{hashcons} function, which inserts a new node into the current DAG, but not
...
\begin{figure}
  ...
  \caption{Number of calls to \mintinline{haskell}{hashcons} plotted against the number of \mintinline{haskell}{add} operations performed.} \label{fig:hashcons}
\end{figure}
...
with a trie.
In our toy example, we use package \texttt{bytestring-trie}.
...
\end{minted}
This looks a bit different because the \mintinline{haskel}{BiMap}
...
To prevent confusion, we rename the hash-consing function to \mintinline{haskel}{triecons}, as follows
\begin{minted}{haskell}
...
we define a new operation \mintinline{haskell}{runCache} to check if
...
\subsubsection{Acknowledgements} We thank NSERC and IBM Canada for supporting this work.
%
% ---- Bibliography ----
%
